...
SELECT
    ...
    sg.carton_j,
    sg.carton_r,
    sg.possession * 100 AS possession_percentage,
    sg.passes_reussies * 100 AS passes_reussies_percentage,
    sg.aeriens_gagnes,
    sg.note
FROM
    statistique_general sg
JOIN
    v_equipe e ON sg.id_equipe = e.id_equipe
WHERE
    sg.categorie = 'general';
...
CREATE OR REPLACE VIEW V_DEFENSE_GENERAL AS
    SELECT
        ...
        SD.NOTE
    FROM
        STATISTIQUE_DEFENSE SD
        JOIN V_EQUIPE E
        ON SD.ID_EQUIPE = E.ID_EQUIPE
    WHERE
        SD.CATEGORIE = 'general';
...
CREATE OR REPLACE VIEW V_ATTAQUE_GENERAL AS
    SELECT
        ...
        SA.NOTE
    FROM
        STATISTIQUE_ATTAQUE SA
        JOIN V_EQUIPE E
        ON SA.ID_EQUIPE = E.ID_EQUIPE
    WHERE
        SA.CATEGORIE = 'general';
